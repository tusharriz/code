...
let obj = {
    data: [],
    add: function (name, age, html, css, js) {
        let studentDetails = {};

        studentDetails["name"] = name;
        studentDetails["age"] = age;
        studentDetails["html"] = html;
        studentDetails["css"] = css;
        studentDetails["js"] = js;
    ...
    marks: function () {
        ...
        for(let i=0; i<=this.data.length-1; i++){
            let arr = this.data[i];
            if(arr["html"]+arr["css"]+arr["js"] > max){
                max = arr["html"]+arr["css"]+arr["js"]
                highestStudent = arr["name"];
            }else{
                if(arr["html"]+arr["css"]+arr["js"] < min){
                    min = arr["html"]+arr["css"]+arr["js"]
                    lowestStudent = arr["name"];
                }
            }
        }
        //Total = [ 288, 276, 295 ]
        console.log(lowestStudent,min);
        console.log(highestStudent,max);

    }
};

obj["add"]("Ram", 21, 100, 91, 97);
obj["add"]("Shayam", 20, 92, 93, 91);
obj["add"]("Hari", 23, 98, 97, 100);
